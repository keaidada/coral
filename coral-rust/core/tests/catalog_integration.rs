...
fn translate_with_catalog_no_issues_when_all_resolves() {
    ...
    assert!(result.spark_sql.contains("FROM default.employees"), "got: {}", result.spark_sql);
}
...
fn translate_with_catalog_flags_unknown_column_with_suggestion() {
    let cat = sample_catalog();
    let result = translate_with_catalog(
        "SELECT e.dpt_id FROM default.employees e",
        &cat,
    )
    .unwrap();
    assert_eq!(result.issues.len(), 1);
    match &result.issues[0] {
        ValidationIssue::UnknownColumn { column, did_you_mean, .. } => {
    ...
    assert!(result.spark_sql.contains("SELECT e.dpt_id"), "got: {}", result.spark_sql);
}
...
fn translate_with_catalog_respects_aliases() {
    ...
    let result = translate_with_catalog(
        "SELECT emp.salary FROM default.employees emp",
        &cat,
    )
    .unwrap();
    assert!(result.issues.is_empty(), "{:?}", result.issues);
